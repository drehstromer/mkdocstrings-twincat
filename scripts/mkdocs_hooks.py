# Generate a JSON schema of the Twincat handler configuration.
...
from mkdocstrings_handlers.twincat._internal.config import TwincatInputConfig, TwincatInputOptions
...
def on_post_build(config: MkDocsConfig, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    @dataclass
    class TwincatHandlerSchema:
        twincat: TwincatInputConfig

    adapter = TypeAdapter(TwincatHandlerSchema)
    ...
    for field in fields(TwincatInputConfig):  # type: ignore[arg-type]
        if f"setting-{field.name}" not in autorefs._primary_url_map:
            _logger.warning(f"Handler setting `{field.name}` is not documented")
    for field in fields(TwincatInputOptions):  # type: ignore[arg-type]
